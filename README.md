...
- **Live file pin** — a file re-read from disk on every prompt, so edits to it stay reflected automatically (the default limit is 128 KB; larger files are visibly truncated).
